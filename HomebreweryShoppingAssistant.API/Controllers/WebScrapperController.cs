...
using HomebreweryShoppingAssistant.WebScrappers;
...
namespace HomebreweryShoppingAssistant.Controllers
{
	...
	public class WebScrapperController : ControllerBase
	{
		...
		public WebScrapperController(AppDbContext context)
		{
			_context = context;
		}
        ...
        */
		...
		public async Task<IActionResult> ScrappingTwojBrowar()
		{
			List<Product> scrappedProducts = TwojBrowarWebScrapper.Run();
			foreach (var scrappedProduct in scrappedProducts)
			{
				var existingProduct = await _context.Products.FirstOrDefaultAsync(p => p.ProductLink == scrappedProduct.ProductLink);
				if (existingProduct == null)
				{
					_context.Products.Add(scrappedProduct);
				}
